 ...
 * @author Michal G.
 */
public class AwtMapGenerator extends JFrame implements KeyListener, ActionListener {
...
//    private static final String NEWLINE = System.getProperty("line.separator");    
    ...
    private AwtCharConstruction charCon;
    
    public static void main(String[] args) {
        //demonstration of static method hiding
             
        System.out.println("parent key map size " + AwtKeyMap.KEY_MAP.size());
        try {
            System.out.println("first value from parent key map " + AwtKeyMap.KEY_MAP.get(33).rendering);
        }
        catch (Exception ex) {
            System.out.println("cannot get first element from parent because " + ex.toString() + " " + ex.getMessage());
        }
        
        System.out.println("en-us key map size " + AwtKeyMap_EN_US.KEY_MAP.size());
        System.out.println("first value from en-us key map " + AwtKeyMap_EN_US.KEY_MAP.get(33).rendering);
        
        System.out.println("sample key map size " + SampleAwtExtendedKeyMap.KEY_MAP.size());
        System.out.println("first value from sample key map " + SampleAwtExtendedKeyMap.KEY_MAP.get(33).rendering);
        try {
...
//        txtOutput.setEditable(false);
        ...
        btnClear = new JButton(AwtMapGenerator.BTN_CLEAR_TEXT);
        btnClear.addActionListener(this);       
        this.btnCopy = new JButton(AwtMapGenerator.BTN_COPY_TEXT);
...
//        System.out.println("key typed event");        
        ...
        charCon = new AwtCharConstruction(displayedChar, vkNumber, isShifted);
        String mapGen = AwtMapGenerator.formatMapPut(asciiNumber, charCon);
...
//        System.out.println("button text " + btn.getText());
        switch (btn.getText()) {
            case AwtMapGenerator.BTN_CLEAR_TEXT:                
//                System.out.println("btnClear click");
                txtInput.setText("");
                txtOutput.setText("");
                break;
            case AwtMapGenerator.BTN_COPY_TEXT:
                txtOutput.selectAll();
                txtOutput.copy();
                break;
            default:                
...
//        sb.append(char_con.keyEventConstant); //should I write this as KeyEvent.VK_ ?
        sb.append("KeyEvent.");
        sb.append(AwtMapGenerator.getVkCode(char_con.keyEventConstant));        
        sb.append(", ");
